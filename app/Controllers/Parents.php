...
class Parents extends BaseController
{
				...
				//		echo var_dump($std);
					...
					echo  view("users/parent/parents_view",$data);
							...
							public function  create_student()
							{
									...
									//	'time'=> 'required'
										...
										if (! $this->validate($rules)){
								 		...
								 		$newdata = [
											...
											'is_parent' => session()->get('is_parent')
									...
									//$this->enroll->insert($coursedata);
										...
										if (session()->get('is_parent')==1){
								 		...
								 		$body = "You are Sucessfuly submit your course application ". " ".$data['courses']['csname']." ". " and your registration id is"." ".$studentno. "Your QR Code " ;
										}
...
public function reate_student_by_parent()
{
			$data[]='';
			$data['courses']= $this->csmodel->getSchoolLeavers();
			return view("users/parent/create_student_by_parent",$data);
}
